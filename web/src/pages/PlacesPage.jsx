...
// on the 12 dated rare conjunctions, grouped by biblical period. Each card links
// to /place/<slug>. Presentational (renders identically server & client).
...
function PlacesPage({ onOpen }) {
  return <NameIndexPage
    ...
    intro={<>The {PLACES.length} distinct biblical places (toponyms) the stellar reading surfaces across the 12 dated rare conjunctions (proofs.json). A place appears when the alignment's occupied letters spell its consonants. Click a place for its profile — Hebrew, gematria, the stellar letters it needs, and the conjunctions where it reads.</>}
    kind="place"
    items={PLACES}
    basePath="/place/"
    onOpen={onOpen}
