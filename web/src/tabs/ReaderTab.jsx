...
function TranslatorTab({date, occ, words, q, setQ, genData, onOpen}){
  ...
  for(const [cons,trans,gloss,pos] of LEX){
    if(cons===he){
      ...
      return {he:cons, disp:displayHe(cons), translit:trans, gloss, pos, len:cons.length, gem:gematria(cons), simp,
        ...
        name:(pos||'').startsWith('n-pr'), theo:/אל|יהו|יאל|יה/.test(cons), compound:/\s/.test(trans),
  ...
  for(const [ahe,aen,asrc] of ANGEL_LEXICON){
    if(ahe===he){
      ...
      return {he:ahe, disp:displayHe(ahe), translit:aen, gloss:'angel — '+aen, pos:'n-pr', len:ahe.length, gem:gematria(ahe), simp,
        pal:isPalindrome(ahe), m37:gematria(ahe)%37===0, name:true, theo:/אל|יה/.test(ahe), compound:false,
...
function GlossPage({word, date, rows, occ, genData, onBack}){
  const w = word;
  const req = w.simp ? [...w.simp] : [];
  ...
  return <>
    ...
    <div className="grid2" style={{alignItems:'start'}}>
      <div className="panel" style={{padding:14}}>
        ...
        <div className="legend">Gold sectors = the simple (zodiac) letters this word needs. Blue = occupied today but not required by this word.</div>
      </div>
        ...
        <table>
          <tbody>
            ...
            <tr><th>Badges</th><td>
              ...
              {w.name && <span className="pill" style={{color:'var(--blue)',borderColor:'var(--blue)'}}>proper name{w.theo?' (theophoric)':''}</span>}
              {w.compound && <span className="pill" style={{color:'var(--warn)',borderColor:'var(--warn)'}}>compound</span>}
              {!w.pal && !w.m37 && !w.angelName && !w.name && !w.compound && <span className="muted">—</span>}
            </td></tr>
