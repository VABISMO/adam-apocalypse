...
// (== figure.slug on NameFicha), so the profile looks up `NAME_WIKI[figure.slug]`.
// Bundled at build time (esbuild JSON import) so the profile renders the Wikipedia block
// (image + description + extract) server-side with no async load — same pattern as
// wiki_content.json for the prophet/mage profiles. Only entries Wikipedia actually has an
// article for are present; the rest fall back to graceful absence on the profile.
